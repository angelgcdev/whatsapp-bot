...
#### Historias Técnicas:
- [ ] **TS-6.1 (Instalación de Prisma 7):** Instalar `prisma@7`, `@prisma/client@7`, `@prisma/adapter-better-sqlite3` y configurar `prisma.config.ts` y `.env` según `docs/PRISMA_7_NESTJS_SETUP.md`.
- [ ] **TS-6.2 (Modelado en schema.prisma):** Definir el modelo `BotResponse` (`id`, `keyword`, `response`, `isActive`, `createdAt`, `updatedAt`).
- [ ] **TS-6.3 (PrismaService & PrismaModule):** Crear el servicio global `PrismaService` y su módulo en NestJS para inyección de dependencias.
- [ ] **TS-6.4 (Migración & Seed Inicial):** Ejecutar migración y poblar la base de datos con los comandos iniciales (`hola`, `menu`, `1`, `2` y fallback).
